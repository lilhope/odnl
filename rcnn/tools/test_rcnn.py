...
def test_rcnn(network, dataset, root_path,subset,split,
              ctx, prefix, epoch,
    ...
    if has_rpn:
        #sym = eval('get_' + network + '_test')(seq_len=46,num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS)
        dataset = Dataset(root_path,dataset,subset,split = split,mode='val')
    ...
    # start detection
    pred_eval(predictor, test_data,roidb, vis=vis, thresh=thresh)
