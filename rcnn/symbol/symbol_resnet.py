...
class symbol_resnet(Symbol):
    ...
    def encoder(self,seq_len,cfg,expression):
         vocab_size = cfg.RNN.VOCAB_SIZE
         num_embed = cfg.RNN.NUM_EMBED
         ...
         cell = cfg.ENCODER_CELL
         outputs,_ = cell.unroll(seq_len,inputs=embed)
         output = mx.symbol.slice_axis(data=outputs,axis=1,begin=-2,end=-1)
         output = mx.symbol.reshape(data=output,shape=(-1,256))
         return output
    def get_dfn_global_feat(self,global_feat,global_roi,sent_feat):
        global_conv_1 = mx.symbol.Convolution(data=global_feat,kernel=(1,1),num_filter=256,stride=(1,1),name="global_conv_1")
        global_conv_1_relu = mx.symbol.Activation(data=global_conv_1,act_type='relu',name="global_conv_1_relu")
        offset_t = mx.contrib.sym.DeformablePSROIPooling(name='offset_t', data=global_conv_1_relu, rois=global_roi, group_size=1, pooled_size=7,
                                                         sample_per_part=4, no_trans=True, part_size=7, output_dim=256, spatial_scale=0.0625)
        offset_global_pool = mx.sym.Pooling(data=offset_t,kernel=(7,7),global_pool=True,pool_type='avg',name='offset_global_pool')
        offset_global_pool = mx.sym.reshape(data=offset_global_pool,shape=(1,256),name='offset_global_pool_reshape')
        offset_origin = mx.sym.concat(offset_global_pool,sent_feat,dim=1)
        offset = mx.sym.FullyConnected(name='offset', data=offset_origin, num_hidden=7 * 7 * 2, lr_mult=0.01)
        offset_reshape = mx.sym.Reshape(data=offset, shape=(-1, 2, 7, 7), name="offset_reshape")

        global_df_roi_pool = mx.contrib.sym.DeformablePSROIPooling(name='deformable_roi_pool', data=global_feat, rois=global_roi,
        ...
        global_conv_2 = mx.symbol.Convolution(data=global_df_roi_pool,kernel=(3,3),stride=(1,1),num_filter=256,name='global_conv_2')
        global_conv_2_relu = mx.sym.Activation(data=global_conv_2,act_type='relu',name='global_conv_2_relu')
        global_feat = mx.sym.Pooling(data=global_conv_2_relu,kernel=(7,7),global_pool=True,pool_type='avg',name="global_feat_pool")
        global_feat = mx.sym.reshape(data=global_feat,shape=(1,256),name='global_feat_reshape')
        return global_feat
    def HighWay_MLP(self,data,name):
        norm = mx.sym.FullyConnected(data=data,num_hidden=1024,name=name+"_norm")
        norm_act = mx.sym.Activation(data=norm,act_type='relu',name=name+'_norm_act')
        gate = mx.sym.FullyConnected(data=data,num_hidden=1024,name=name+'_gate')
        gate_act = mx.sym.Activation(data=gate,act_type='relu',name=name+'_gate_act')
        mlp_gate_and_norm = norm_act * gate_act
        mlp_gate_and_input = (1-gate_act) * data
        return mlp_gate_and_norm + mlp_gate_and_input
        
    def get_symbol(self, seq_len,cfg, is_train=True):
        ...
        conv_feat = self.get_resnet_v1_conv4(data_bn)
        # res5
        ...
        if is_train:
            ...
            if cfg.TRAIN.CXX_PROPOSAL:
                rois = mx.contrib.sym.Proposal(
                    ...
                    feature_stride=cfg.RPN_FEAT_STRIDE, scales=tuple(cfg.ANCHOR_SCALES),
                    ratios=tuple(cfg.ANCHOR_RATIOS),
                    rpn_pre_nms_top_n=cfg.TRAIN.RPN_PRE_NMS_TOP_N, rpn_post_nms_top_n=cfg.TRAIN.RPN_POST_NMS_TOP_N,
                    threshold=cfg.TRAIN.RPN_NMS_THRESH, rpn_min_size=cfg.TRAIN.RPN_MIN_SIZE)
            else:
                rois = mx.sym.Custom(
                    cls_prob=rpn_cls_act_reshape, bbox_pred=rpn_bbox_pred, im_info=im_info, name='rois',
                    op_type='proposal', feat_stride=cfg.RPN_FEAT_STRIDE,
        ...
        #region of interest feat
        roi_pool = mx.symbol.ROIPooling(
            name='roi_pool', data=conv_new_1_relu, rois=rois, pooled_size=(7, 7), spatial_scale=0.0625)
        roi_conv = mx.symbol.Convolution(data=roi_pool,kernel=(3,3),stride=(1,1),num_filter=256,name='roi_conv')
        roi_conv_relu = mx.symbol.Activation(data=roi_conv,act_type='relu',name='roi_conv_relu')
        roi_conv_pool = mx.symbol.Pooling(data=roi_conv_relu,kernel=(7,7),global_pool=True,pool_type='avg',name='roi_conv_pool')
        roi_feat = mx.symbol.reshape(data=roi_conv_pool,shape=(-1,256),name="roi_feat_reshape")
        #sent feat
        sent_feat = self.encoder(seq_len,cfg,expression)
        #global region(jointly predicted by global image feat and sent_feat)
        global_feat = self.get_dfn_global_feat(conv_new_1_relu,global_roi,sent_feat)
        #broadcast and concat
        if is_train:
            sent_feat = mx.sym.broadcast_to(data=sent_feat,shape=(cfg.TRAIN.BATCH_ROIS,256))
            global_feat = mx.sym.broadcast_to(data=global_feat,shape=(cfg.TRAIN.BATCH_ROIS,256))
        else:
            sent_feat = mx.sym.broadcast_to(data=sent_feat,shape=(cfg.TEST.RPN_POST_NMS_TOP_N,256))
            global_feat = mx.sym.broadcast_to(data=global_feat,shape=(cfg.TEST.RPN_POST_NMS_TOP_N,256))
        #feat = mx.sym.concat(roi_feat,sent_feat,global_feat,dim=1)
        feat = roi_feat + sent_feat + global_feat
        # 2 fc
        fc_new_1 = mx.symbol.FullyConnected(name='fc_new_1', data=feat, num_hidden=1024)
        fc_new_1_relu = mx.sym.Activation(data=fc_new_1, act_type='relu', name='fc_new_1_relu')

        #fc_new_2 = mx.symbol.FullyConnected(name='fc_new_2', data=fc_new_1_relu, num_hidden=1024)
        fc_new_2_relu = self.HighWay_MLP(data=fc_new_1_relu,name='Highway_1')
    ...
    def init_weight_rcnn(self, cfg, arg_params, aux_params):
        ...
        arg_params['bbox_pred_weight'] = mx.random.normal(0, 0.001, shape=self.arg_shape_dict['bbox_pred_weight'])
    ...
    def init_weight_extra(self,cfg,arg_params,aux_params):
        ...
        arg_params['global_conv_1_weight'] = mx.random.normal(0,0.01,shape=self.arg_shape_dict['global_conv_1_weight'])
        arg_params['global_conv_1_bias'] = mx.nd.zeros(shape= self.arg_shape_dict['global_conv_1_bias'])
        arg_params['global_conv_2_weight'] = mx.random.normal(0,0.01,shape=self.arg_shape_dict['global_conv_2_weight'])
        arg_params['global_conv_2_bias'] = mx.nd.zeros(shape= self.arg_shape_dict['global_conv_2_bias'])
     

    def init_weight(self, cfg, arg_params, aux_params):
        self.init_weight_rpn(cfg, arg_params, aux_params)
        self.init_weight_rcnn(cfg, arg_params, aux_params)
        self.init_weight_extra(cfg,arg_params,aux_params)
