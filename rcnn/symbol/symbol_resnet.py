...
def get_resnet_train(seq_len,num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    feat = mx.symbol.add_n(roi_pool,sent)
    # res5
    unit = residual_unit(data=feat, num_filter=filter_list[3], stride=(2, 2), dim_match=False, name='stage4_unit1')
...
def get_resnet_test(seq_len,num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    data = mx.symbol.Variable(name="data")
    im_info = mx.symbol.Variable(name="im_info")
    expression = mx.symbol.Variable(name='expression')
