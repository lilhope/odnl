...
def sample_rois(rois, fg_rois_per_image, rois_per_image, num_classes,
    ...
    # foreground RoI with FG_THRESH overlap
    pos_indexes = np.where((overlaps >= config.TRAIN.FG_THRESH) &(labels==1))[0]
    neg_indexes = np.where((overlaps >= config.TRAIN.FG_THRESH) &(labels==2))[0]
    
    #print(pos_indexes)
    #print(neg_indexes)
    # Sample foreground regions without replacement
    if len(pos_indexes) == 0:
        print('zero positive samples get,will random choice from bboxes as sample')
        pos_indexes = np.where(labels==1)[0]
    pos_indexes = choice_or_pad(pos_indexes,fg_rois_per_image/2)
    if len(neg_indexes) == 0:
        print('negative positive samples get,will random choice from bboxes as sample')
        pos_indexes = np.where(labels==2)[0]
    neg_indexes = choice_or_pad(neg_indexes,fg_rois_per_image/2)
    # Select background RoIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_indexes = np.where((overlaps < config.TRAIN.BG_THRESH_HI) & (overlaps >= config.TRAIN.BG_THRESH_LO))[0]
    # Compute number of background RoIs to take from this image (guarding against there being fewer than desired)
    ...
    # indexes selected
    fg_indexes = np.append(pos_indexes,neg_indexes)
    keep_indexes = np.append(fg_indexes, bg_indexes)
...
def choice_or_pad(indexs,nums):
    """if num of indexs > nums, random choice nums of samples from indexs
        otherwise pad indexs to nums size
    """
    if len(indexs)==nums:
        return indexs
    elif len(indexs)>nums:
        indexs = npr.choice(indexs,size=nums,replace=False)
    elif len(indexs)<nums:
        gap = nums - len(indexs)
        gap_indexs = npr.choice(indexs,gap,replace=True)
        indexs = np.append(indexs,gap_indexs)
    return indexs
