...
def get_rpn_batch(roidb):
    ...
    bbox = roidb[0]['bbox']
    candidates = roidb[0]['candidates']
    num_bbox = bbox.shape[0]
    num_cans = candidates.shape[0]
    gt_boxes = np.empty((num_bbox + num_cans,5),dtype=np.float32)
    gt_boxes[0:num_bbox,0:4] = roidb[0]['bbox']
    gt_boxes[0:num_bbox,4] = np.float32(1.0)
    gt_boxes[num_bbox:,0:4] = roidb[0]['candidates']
    gt_boxes[num_bbox:,4] = np.float32(2.0)
    #expression (word ids)
    expression = roidb[0]['expression']
    expression = np.array(expression,dtype=np.float32)
    expression = expression[np.newaxis,:]
    data = {'data': im_array,
            'im_info': im_info,
            'expression':expression}
